...
extern "C" {
#endif
void* nefu_lvgl_pool_alloc(size_t size);
#ifdef __cplusplus
}
#endif
...
#define LV_STDARG_INCLUDE       <stdarg.h>
    ...
    /*Instead of an address give a memory allocator that will be called to get a memory pool for LVGL. E.g. my_malloc*/
        ...
        /*Allocate the LVGL heap from the nefuOS kernel heap at runtime so the
         *1 MiB pool is not embedded in the kernel image (keeps kernel.bin small).*/
        #ifdef __cplusplus
        extern "C" {
#endif
        extern void* nefu_lvgl_pool_alloc(size_t size);
#ifdef __cplusplus
        }
#endif
        #define LV_MEM_POOL_ALLOC(size) nefu_lvgl_pool_alloc((size))
    #endif
