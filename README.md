...
### First-boot setup wizard (BIOS/UEFI password gate)

* On the very first boot (no `/var/lib/nefuos/firstboot` marker) nefuOS runs a
  full-screen setup wizard before anything else:
  * **username** (default `user`)
  * **password** and **confirm password** (SHA-256 hex is stored in the UEFI
    config block; the plaintext never touches disk, logs or the ISO)
  * **browser engine** choice: `minijs` (built-in mini JS engine) or
    `noscript` (HTML only)
* Finishing the wizard writes `/etc/nefu.conf` (`browser_engine=`), creates
  the `/var/lib/nefuos/firstboot` marker and drops the system into the lock
  screen. The desktop can only be reached by typing the password you just set.
* The lock screen compares `SHA-256(input)` against the stored hash
  (`strcmp` on the 64-hex-digit string); wrong input shows a red hint and
  clears the field.
...
| [stb\_image](https://github.com/nothings/stb)     | 2.30                                                                          | MIT / public domain | JPEG/PNG/GIF/BMP decode for the Image Viewer and the Browser image chain          | wrapped in `third_party/stb_image_wrap.cpp`; `stbi__malloc` hooked to `nefu::kalloc` for the bare kernel                                                                                                |
...
Third-party license texts are kept at `third_party/licenses/`.
